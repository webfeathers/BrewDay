...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE brew_schedule SET brew_actual_started_date=%s WHERE brew_id=%s",
                       GetSQLValueString($_POST['brew_start_date'], "date"),
  ...
  $updateGoTo = "brewSchedule.php?started=true";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="container">
	<h1>Start a brew <a href="/addBrew.php" class="btn btn-primary pull-right">Cancel</a> </h1>
	<?php do { ?>
		<h3>Ready to brew?</h3>
		<p>A <?php echo $row_thisBrew['batch_size']; ?> batch of <?php echo $row_thisBrew['recipe_name']; ?> is scheduled to start on <?php echo $row_thisBrew['brew_planned_start']; ?></p>
		<form action="<?php echo $editFormAction; ?>" name="form" method="POST">
			<p>Set the brew start to today (<?php echo date("Y-m-d") ?>) by clicking
				<button type="submit" class="btn btn-primary">Brew It</button>
			</p>
			<input type="hidden" name="brew_start_date" value="<?php echo date("Y-m-d") ?>"/>
